...
func (c *Controller) pollNewDeployment(deployment Deployment) {
	...
	fmt.Println("  compose users:create --deployment " + deployment.Name + " --database " + deployment.Databases[0].Name + " -u <username>")
	fmt.Println("")
	fmt.Println("To connect to your database, run:")
	fmt.Println("  compose " + deployment.CurrentPrimary + "/" + deployment.Databases[0].Name + " -u <username>" + " -p")
	fmt.Println("")
