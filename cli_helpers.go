...
func requireArguments(c *cli.Context, argumentsSlice []string, errorMessages []string) error {
	...
	if err {
		if !replMode {
			fmt.Println("\nMissing arguments, for more information, run: compose " + c.Command.Name + " --help\n")
		} else {
...
func findClosestCommand(context *cli.Context, command string) {
	...
	if !replMode {
		fmt.Println(" ! `" + command + "` is not a compose command.")
		fmt.Println(" ! See `compose help` for a list of available commands")
	} else {
